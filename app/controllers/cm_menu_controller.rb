class CmMenuController < ApplicationController
  ...
  before_filter :find_item, only: [:destroy, :update, :edit]
  before_filter :build_new_item, only: [:new, :create]
  ...
  def rebuild_menu
    ...
    CmItem.transaction do
      ...
      elsif (Setting.plugin_custom_menu || {})['use_cache']
  ...
  def build_tree(tree, menu, parent=nil)
    errors_list = {}
    tree.each do |k, t|
      item = CmItem.new_subclass_instance(t[:type], { code: t[:code], menu: menu, caption: t[:caption], custom_url: t[:custom_url], options: { class: t[:options_class] }, logged: t[:logged], visibility: t[:visibility] })
